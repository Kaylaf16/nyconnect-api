...
router.post("/filterUsersBySkill", (req, res) => {
  ...
  const query =
    ...
    ` as decimal(10,6)) ) - radians(${parseFloat(long)}) ) + sin( radians(${parseFloat(lat)}) ) * sin( radians( cast(substring_index(u.location, ',', 1)` +
    " as decimal(10,6)) ) ) ) )) AS distance FROM user u" +
    ` inner join user_skills us on u.id = us.user_id inner join skill s on s.id = us.skill_id and s.skill = '${skill}'group by u.name HAVING distance < ${parseInt(radius)} order by` +" distance;";
